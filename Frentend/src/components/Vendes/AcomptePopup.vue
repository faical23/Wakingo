<template>
  <div class="PopupNewClient Acompte">
    <div class="PopupNewClient__title">
      <p>Acompte client</p>
      <svg
      @click="$emit('RemovePopupAcompte')"
        version="1.1"
        id="Capa_1"
    ...
    <div class="PopupNewClient__information">
        <div class="DateAndLibillé">
            <div class="PopupNewArticle_Information__Failed">
                        <h6>Date * :</h6>
                        <input type="Date" v-model="DateCompte" />
                        <span v-if="SubmitForm && DateCompte == '' " class="MessageErrorFiled ">Ce champ est obligatoire.</span>
            </div>
            <div class="PopupNewArticle_Information__Failed">
                        <h6>Libillé * :</h6>
                        <input type="text" v-model="Libillé" />
                        <span v-if="SubmitForm && Libillé == '' " class="MessageErrorFiled ">Ce champ est obligatoire.</span>
            </div>
        </div>
        <div class="SoldQualitéMontantReste">
            <div class="PopupNewArticle_Information__Failed">
                        <h6>Solde dû:</h6>
                        <input class="Solde" type="text"  v-model="Solde" disabled />
            </div>
            <div class="PopupNewArticle_Information__Failed">
                        <h6>Quotité * :</h6>
                        <div class="Qualité">
                            <input type="text" v-model="Quotité" @keyup="GetDataFromVuex" />
                            <input type="text" placeholder="%" disabled />
                        </div>
                        <span v-if="SubmitForm && Quotité== '' " class="MessageErrorFiled ArticleErrorPoupFaild">Ce champ est obligatoire.</span>
            </div>
            <div class="PopupNewArticle_Information__Failed">
                        <h6>Montant à régler *</h6>
                        <input type="text" v-model="MontantRégler" />
                        <span v-if="SubmitForm && MontantRégler == '' " class="MessageErrorFiled ArticleErrorPoupFaild">Ce champ est obligatoire.</span>
            </div>
            <div class="PopupNewArticle_Information__Failed">
                        <h6>Reste à régler :</h6>
                        <input type="text" v-model="ResteRégler" class="ResteRégler" disabled/>
            </div>
        </div>
    ...
    <div class="PopupNewClient__btns">
      <button class="Btn__Firmer" @click="$emit('RemovePopupAcompte')">
        <svg
          version="1.1"
          id="Capa_1"
          xmlns="http://www.w3.org/2000/svg"
...
export default {
  ...
  data: () => ({
    ...
    DateEcheance:'',
    ModePiament:'Veuillez sélectionner',
    ModePaimentOption:[ 
        {
